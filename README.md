...
- **普通 push / PR 到 main**：自动编译，产物 jar 作为 Artifact 上传（在对应 workflow run 页面的 Artifacts 区下载）。
- **打 tag 触发发布**：推送形如 `v1.0.0` 的 tag 会自动创建 GitHub Release 并附带 jar。
...
也可在仓库的 **Actions** 页面用 **Run workflow** 手动触发一次构建。